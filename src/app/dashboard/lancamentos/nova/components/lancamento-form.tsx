...
export function LancamentoForm({ parceirosOptions }: { parceirosOptions: ParceiroOption[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!dataLancamento || !emailParceiro || !qtd) {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push(`/dashboard/extrato?email=${encodeURIComponent(emailParceiro)}`)
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-2">
        <label htmlFor="dataLancamento" className="text-sm font-medium text-slate-600 flex items-center gap-2">
      ...
      <div className="flex flex-col gap-2 mt-4">
        <SearchableSelect
          ...
          value={emailParceiro}
          onChange={(v) => setEmailParceiro(v)}
        />
      </div>

      {/* CNPJ Input */}
      <div className="flex flex-col gap-2 mt-2">
        <label htmlFor="cnpjEmpresa" className="text-sm font-medium text-slate-600">

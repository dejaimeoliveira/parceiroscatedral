...
export async function submitIndicacaoPublica(formData: FormData) {
  ...
  const formattedToken = token.length === 32 
  ...
  if (error) {
    ...
    return { error: 'Falha ao registrar indicação. Detalhes: ' + error.message }
  }

  // Enviar email de notificação
  try {
    console.log('Iniciando envio de email para parceiro e admin (Pública)...', { parceiroEmail: parceiro.email, empresa });
    ...
    console.error('Erro crítico ao disparar a função de email de indicação (Pública):', err)
  }
